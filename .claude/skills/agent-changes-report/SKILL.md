...
description: Generate a unified report of all AI agent activity (Claude Code + Cursor) for this project in a given time range. Use when the user asks "今天 AI 做了什么", "生成变更报告", "agent changes report", or similar. Outputs to docs/agent-changes/.
user-invocable: true
...
综合查询 **Claude Code** 和 **Cursor** 在当前项目中的 AI 工作记录，结合 `git log`，生成统一 Markdown 报告输出到 `docs/agent-changes/`。
...
```bash
SKILL_SCRIPTS="/Users/yang.zhou/workspace/open-vera/.claude/skills/agent-changes-report/scripts"
...
1. **汇总表格**：Claude sessions / Cursor sessions / git commits 数量对比
2. **Claude Code Sessions**：每个 session 的 prompt 列表 + 修改文件
...
```bash
python3 /Users/yang.zhou/workspace/open-vera/.claude/skills/agent-changes-report/scripts/unified_report.py [用户传入的参数]
```
