...
- [ ] **M1** 记忆自动提取 — agent 执行过程中自动识别高价值信息存入 semantic memory
- [ ] **M2** 记忆自动整理 — 定期去重、合并相似记忆、清理过期记忆
- [ ] **M3** 记忆压缩 — 大量记忆自动聚类压缩为高层摘要
- [ ] **M4** 记忆衰减 — 基于访问频率的重要性衰减机制（未被检索的记忆权重降低）
- [ ] **M5** 记忆关联图谱 — 记忆之间建立关联关系，支持关联检索
- [ ] **M6** 记忆增强测试（10+ tests：自动提取准确性、去重、压缩质量、衰减曲线）
