...
function main() {
  ...
  versions.sort(
    (a, b) =>
      (orderMap.get(a.id as any) ?? 99) - (orderMap.get(b.id as any) ?? 99)
  );
  ...
  if (fs.existsSync(DOCS_DIR)) {
    ...
    for (const locale of localeDirs) {
      ...
      for (const filename of docFiles) {
        ...
        if (!version) {
          console.warn(
            `  Skipping doc ${locale}/${filename}: could not determine version`
          );
          continue;
        }
    ...
    console.log(
      `  Found ${totalDocFiles} doc files across ${localeDirs.length} locales`
    );
  } else {
