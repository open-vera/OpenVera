...
import { isBuddyEnabled } from '../utils/sourceFeatures.js';
...
function getCompanionLabel(companion: ReturnType<typeof getCompanion>): string {
  if (!companion) return ''
...
export function companionReservedColumns(terminalColumns: number, speaking: boolean): number {
  ...
  const nameWidth = stringWidth(getCompanionLabel(companion));
...
export function CompanionSprite(): React.ReactNode {
  ...
  const companionLabel = getCompanionLabel(companion);
  const color = RARITY_COLORS[companion.rarity];
  const colWidth = spriteColWidth(stringWidth(companionLabel));
  ...
  if (columns < MIN_COLS_FOR_FULL_SPRITE) {
    ...
    const label = quip ? `"${quip}"` : focused ? ` ${companionLabel} ` : companionLabel;
  ...
  const spriteColumn = <Box flexDirection="column" flexShrink={0} alignItems="center" width={colWidth}>
      ...
      <Text italic bold={focused} dimColor={!focused} color={focused ? color : undefined} inverse={focused}>
        {focused ? ` ${companionLabel} ` : companionLabel}
      </Text>
    </Box>;
  if (!reaction) {
